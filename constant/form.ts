...
//     [key: string]: Array<{ fieldName: string; name: string; type: string; option?: any }>;
// }

export const getSpecification = async (): Promise<ISpecification> => {

    const URL = "https://star-tech-back-end.vercel.app/api/v1/category/";
    const res = await fetch(URL);
    const category = (await res.json() as IResponseType<ICategory>).data;
    ...
    const Specification: ISpecification = {
        product: [
            ...
            { fieldName: "Category", name: "category_id", type: "select", option: category },
